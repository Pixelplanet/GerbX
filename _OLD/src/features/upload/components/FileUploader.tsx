...
interface FileUploaderProps {
    onFilesAccepted: (files: File[] | File) => void;
...
export const FileUploader: React.FC<FileUploaderProps> = ({ onFilesAccepted, isProcessing }) => {
    ...
    const validateFile = (file: File) => {
        ...
        const isGerber = ['.gbr', '.gtl', '.gbl', '.gbs', '.gts', '.gbo', '.gto', '.drl', '.xln', '.txt'].some(ext => file.name.toLowerCase().endsWith(ext));

        if (isZip || isXml || isSvg || isGerber) {
            return true;
        }
        return false;
    };
    ...
    const handleDrop = useCallback((e: React.DragEvent) => {
        ...
        if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
            ...
            if (validFiles.length > 0) {
                // If single file & zip/xml/svg, pass as single? Or just always array?
                // The prompt asked for "multiple files instead of forcing zip".
                // So we should pass array.
                onFilesAccepted(validFiles);
            } else {
                setError('Invalid file format. Please use Gerber files, ZIP, SVG, or XML.');
            }
        }
    }, [onFilesAccepted]);
        ...
        `}
            >
                ...
                <input
                    type="file"
                    className="hidden"
                    accept=".zip,.xml,.svg,.gbr,.gtl,.gbl,.gbs,.gts,.gbo,.gto,.drl,.xln,.txt"
