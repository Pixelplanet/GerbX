...
import { Maximize2, ZoomIn, ZoomOut, Move, Download, FileCode } from 'lucide-react';
        ...
        >
            ...
            <div className="absolute bottom-4 right-4 flex flex-col gap-2 z-30">
                <button
                    ...
                    className="p-2 glass hover:bg-white/10 text-brand-accent hover:text-white rounded-lg transition-all group relative"
                    title="Export to SVG"
                >
                    <Download size={18} />
                    <span className="absolute right-full mr-2 top-1/2 -translate-y-1/2 px-2 py-1 bg-black/80 text-white text-xs rounded opacity-0 group-hover:opacity-100 whitespace-nowrap pointer-events-none transition-opacity">
                        Export SVG
                    </span>
                </button>
                <button
                    onClick={() => downloadXcs(layers)}
                    className="p-2 glass hover:bg-white/10 text-brand-accent hover:text-white rounded-lg transition-all group relative mb-2"
                    title="Export to XCS"
                >
                    <FileCode size={18} />
                    <span className="absolute right-full mr-2 top-1/2 -translate-y-1/2 px-2 py-1 bg-black/80 text-white text-xs rounded opacity-0 group-hover:opacity-100 whitespace-nowrap pointer-events-none transition-opacity">
            ...
            <div className="flex-1 relative overflow-hidden bg-[#0a0a0a] rounded-3xl border border-white/5 inner-shadow">
                ...
                >
                    ...
                    {visibleLayers.sort((a, b) => {
                        ...
                        if (layer.raster) {
                            // Use the high-quality JPG raster if available
                            return (
                                <image
                                    key={layer.id}
                                    href={layer.raster}
                                    ...
                                    style={{
                                        ...layerStyle,
                                        mixBlendMode: viewMode === 'xray' ? 'screen' : 'screen' // Using screen to allow dark substrate to show through if JPG is dark-on-white? 
                                        // Actually, if it's color-on-white JPG, multiply is better.
                                        // If it's color-on-black, screen is better.
                                        // My rasterizer uses white background and color fill.
                                    }}
